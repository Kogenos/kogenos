<!--
**AlexandrRogachev/AlexandrRogachev** — специальный репозиторий для моего GitHub-профиля.
-->

<div align="center">
<div align="center">
  <pre>
  █████╗ ██╗     ███████╗██╗  ██╗ █████╗ ███╗   ██╗██████╗ ██████╗ ███████╗
  ╚════╝ ╚═╝     ╚══════╝╚═╝  ╚═╝ ╚════╝ ╚══╝   ╚═╝╚═════╝ ╚═════╝ ╚══════╝
  </pre>
  <p>
    <img src="https://img.shields.io/badge/Go-00ADD8?style=for-the-badge&logo=go&logoColor=white" />
    <img src="https://img.shields.io/badge/C-00599C?style=for-the-badge&logo=c&logoColor=white" />
    <img src="https://img.shields.io/badge/C++-00599C?style=for-the-badge&logo=cplusplus&logoColor=white" />
    <img src="https://img.shields.io/badge/PostgreSQL-4169E1?style=for-the-badge&logo=postgresql&logoColor=white" />
    <img src="https://img.shields.io/badge/Docker-2496ED?style=for-the-badge&logo=docker&logoColor=white" />
    <img src="https://img.shields.io/badge/Linux-FCC624?style=for-the-badge&logo=linux&logoColor=black" />
  </p>
  <h4>⚡ Junior Golang Developer | Open to work</h4>
</div>
</div>

<div align="center">
  <!-- Терминальная анимация: анимированная печать команд в стиле терминала -->
  <img src="https://readme-typing-svg.demolab.com?font=Fira+Code&size=18&duration=3000&pause=500&color=33FF33&center=true&vCenter=true&width=550&lines=~%24+go+run+server.go;Server+started+on+port+8080;~%24+git+push+origin+main;Everything+up-to-date;~%24+ls+projects/;brickgame++tetris++docker;~%24+echo+%22Ready+for+work%22;Ready+for+work" alt="Terminal animation" />
</div>

---

## 🚀 Обо мне

Я начинающий **Golang-разработчик** с сильным бэкграундом в системном программировании (C/C++) и администрировании Linux (в т.ч. через виртуальные машины). Люблю писать чистый, тестируемый код и разбираться, как всё работает под капотом.

- 🔭 Сейчас углубляюсь в микросервисы, gRPC и Kubernetes.
- 🌱 Постоянно учусь через создание реальных проектов.
- 🎯 Ищу стажировку / Junior-позицию в Go-команду.

---

## 🛠️ Мой стек

<div align="center">
  <img src="https://img.shields.io/badge/Go-00ADD8?style=for-the-badge&logo=go&logoColor=white" />
  <img src="https://img.shields.io/badge/C-00599C?style=for-the-badge&logo=c&logoColor=white" />
  <img src="https://img.shields.io/badge/C++-00599C?style=for-the-badge&logo=cplusplus&logoColor=white" />
  <img src="https://img.shields.io/badge/PostgreSQL-4169E1?style=for-the-badge&logo=postgresql&logoColor=white" />
  <img src="https://img.shields.io/badge/Docker-2496ED?style=for-the-badge&logo=docker&logoColor=white" />
  <img src="https://img.shields.io/badge/Linux-FCC624?style=for-the-badge&logo=linux&logoColor=black" />
  <img src="https://img.shields.io/badge/Git-F05032?style=for-the-badge&logo=git&logoColor=white" />
  <img src="https://img.shields.io/badge/GitLab%20CI-FC6D26?style=for-the-badge&logo=gitlab&logoColor=white" />
  <img src="https://img.shields.io/badge/JWT-000000?style=for-the-badge&logo=jsonwebtokens&logoColor=white" />
</div>

---

## 🧰 Мои инструменты

<div align="left">
  
  ⌨️ **Редакторы:** VS Code · GoLand  
  💻 **ОС:** macOS (основная) · Linux (виртуалки / серверы)  
  🐳 **Контейнеризация:** Docker · Docker Compose  
  🔧 **Сборка:** Make · CMake  
  📦 **Базы данных:** PostgreSQL · pgAdmin · SQLite  
  🌐 **Веб-сервер / прокси:** nginx  
  🔁 **CI/CD:** GitLab CI · GitHub Actions  
  🛡️ **Сеть:** iptables (Linux), базовое администрирование  
  🧪 **Тестирование:** GTest / Check · testify · httptest  
  🐙 **Контроль версий:** Git (CLI) · GitLab · GitHub  

</div>

---

## 🔥 Ключевые проекты

| Проект | Описание | Технологии |
|--------|----------|-------------|
| **Консольные утилиты на Go** | Калькулятор, топ K слов, пересечение списков, журнал посещений | Go (std lib), unit-тесты |
| **Конкурентное программирование** | Горутины с WaitGroup, конвейер (генератор → квадрат), тикер на time.Sleep | Go (goroutines, channels, signals) |
| **Веб-приложение** | REST API с ИИ (минимакс), JWT-авторизация, PostgreSQL, слоистая архитектура | Go, net/http, JWT, pgx, uber-fx |
| **Системное программирование на C** | string.h + sprintf/sscanf, decimal (128 бит), матричная библиотека, Тетрис с ncurses | C11, Makefile, GTest/Check |
| **ООП на C++** | Класс для работы с матрицами (конструкторы, операторы, методы) | C++20, Google Style, GTest |
| **Библиотека контейнеров на C++** | Реализация аналогов STL: list, map, queue, set, stack, vector | C++, шаблоны, итераторы, GTest |
| **DevOps: Linux + Docker + CI/CD** | Настройка сетей, Docker-образ с веб-сервером, GitLab CI (сборка, тесты, деплой) | Bash, Docker, GitLab CI, nginx |
| **SQL** | Сложные запросы, индексы, транзакции, триггеры, хранимые функции | PostgreSQL, PL/pgSQL |

> *Код всех проектов выложен в открытых репозиториях или предоставляется по запросу.*

---

## 📫 Связь со мной

<div align="center">
  <a href="https://t.me/Kogeno">
    <img src="https://img.shields.io/badge/Telegram-26A5E4?style=for-the-badge&logo=telegram&logoColor=white" />
  </a>
  <a href="mailto:sasha.rogachiov@yandex.ru">
    <img src="https://img.shields.io/badge/Email-D14836?style=for-the-badge&logo=gmail&logoColor=white" />
  </a>
</div>

---

<div align="center">
  <img src="https://komarev.com/ghpvc/?username=AlexandrRogachev&label=Profile%20views&color=0e75b6&style=flat" alt="Profile views" />
  <br/>
  <i>“Make it work, make it right, make it fast.”</i>
</div>
